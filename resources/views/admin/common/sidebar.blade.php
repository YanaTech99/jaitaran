<div class="sidebar-wrapper">
   <div>
      <div class="logo-wrapper">
         <a href="#"><img class="img-fluid for-light" src="{{url('/')}}/{{$settings['websiteLogo']}}" alt="" style="width:100%;height:40px;display:block;margin:auto;"><img class="img-fluid for-dark" src="{{url('backend/assets/images/logo/small-white-logo.png')}}" alt=""></a>
         <div class="back-btn"><i class="fa fa-angle-left"></i></div>
      </div>
      <div class="logo-icon-wrapper"><a href="{{url('/')}}/admin"><img class="img-fluid" src="{{url('backend/assets/images/logo-icon.png')}}" alt=""></a></div>
      <nav class="sidebar-main">
         <div class="left-arrow" id="left-arrow"><i data-feather="arrow-left"></i></div>
         <div id="sidebar-menu">
            <ul class="sidebar-links" id="simple-bar">
               <li class="back-btn">
                  <a href="#"><img class="img-fluid" src="{{url('backend/assets/images/logo-icon.png')}}" alt=""></a>
                  <div class="mobile-back text-end"><span>{{$controller::message("Back")}}</span><i class="fa fa-angle-right ps-2" aria-hidden="true"></i></div>
               </li>
               @php($parents = $controller::getDataOrderBy("menus","sortOrder","ASC"))
               @foreach($parents as $parent)
               @php($rolesArrayMenuCheck = json_decode($parent['roleId'],true))
               @if(!empty($rolesArrayMenuCheck))
               @if(!in_array(Session::get('roleId'),$rolesArrayMenuCheck))
               @continue;
               @endif
               @else
               @continue;
               ...
               <li class="sidebar-list">
                  @if($parent['link']!="")
                  @php($link = url("/")."/admin".$parent['link'])
                  @else
                  @php($link = "#")
                  @endif
                  ...
                  @if(!empty($module))
                  @php($childArray[] = strtolower($module[0]['moduleName']))
                  @else
                  @php($childArray[] = $child['link'])
                  @endif
                  @endforeach
                  @endif
                  @php($page = (Request::segment(3)!="")?Request::segment(3):Request::segment(2))
                  @php(in_array($page,$childArray)?$show=1:$show=0)
                  ...
                  <ul class="sidebar-submenu" style="{{($show==1)?'display: block':'display: none'}}">
                     ...
                     @php($module = $controller::getDataWhere("modules","moduleId",$child['moduleId']))
                     @if(!empty($module))
                     <li><a class="{{$page==strtolower($module[0]['moduleName'])?'active':''}}" href="<?php echo url("/").'/admin/page/'.strtolower($module[0]['moduleName']);?>">{{$controller::message($child['menuHeading'])}}</a></li>
                     @else
                     <li><a class="{{($page==$child['link'])?'active':''}}" href="<?php echo url("/").'/admin/'.$child['link'];?>">{{$controller::message($child['menuHeading'])}}</a></li>
                     @endif
                     @endforeach
                  </ul>
                  @endif
